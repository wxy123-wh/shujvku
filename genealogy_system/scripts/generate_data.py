...
# Keep some negative cases for the "older than 50 and no spouse" query.
UNMARRIED_MALE_INTERVAL = 7
...
def build_spouse_couples(
    ...
    for index, male in enumerate(males):
        male_id = member_id(male)
        if len(males) > 1 and index % UNMARRIED_MALE_INTERVAL == 0:
            continue
        if male_id in paired_member_ids:
            continue
        for shift in range(len(females)):
            female = females[(index + shift) % len(females)]
            female_id = member_id(female)
            if female_id in paired_member_ids:
                continue
            if can_marry(male, female, parent_sets, existing_pairs):
                couples.append((male, female))
                existing_pairs.add(spouse_pair(male, female))
                paired_member_ids.update((male_id, female_id))
                break
    return couples
